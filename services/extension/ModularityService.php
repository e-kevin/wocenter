...
class ModularityService extends Service
{
    ...
    {
        return isset($this->service->getLoad()->getAllExtensionConfig()[Yii::$app->id]['modules'])
            ? $this->service->getLoad()->getAllExtensionConfig()[Yii::$app->id]['modules']
            : [];
    }
        ...
        switch ($type) {
            case 'install':
                return Wc::getOrSet([
                    ...
                    // 为模块添加功能扩展配置信息
                    if (isset($this->service->getController()->getInstalledConfig()['modules'])) {
                        $installedControllerConfig = $this->service->getController()->getInstalledConfig()['modules'];
                        foreach ($config as $moduleId => &$row) {
                            if (isset($installedControllerConfig[$moduleId])) {
                                $row['controllerMap'] = array_merge(
                                    isset($row['controllerMap']) ?: [],
                                    $installedControllerConfig[$moduleId]
                                );
                            }
                        }
                    }
                ...
                break;
            case 'uninstall':
                return Wc::getOrSet([$appId, self::CACHE_UNINSTALL_MODULES], function () {
                    /** @var Module $moduleModel */
                    $moduleModel = Yii::createObject($this->moduleModel);
                    // 已经安装的模块ID数组
                    $installedModuleIds = $moduleModel->getInstalledModuleId();
                    // 系统存在的模块ID数组
                    $existModuleIds = array_keys($this->getAllConfig());
