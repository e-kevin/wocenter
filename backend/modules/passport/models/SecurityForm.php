...
class SecurityForm extends Model
{
    ...
    public function findPassword()
    {
        if ($this->validate()) {
            /** @var User $user */
            $user = User::findByIdentity($this->email);
            if (false == $user) {
            ...
            if ($actionService->checkLimit('find_password', User::tableName(), $user->id)) {
                ...
                if ($user->save(false)) {
                    ...
                    Wc::$service->getLog()->create('find_password', User::tableName(), $user->id, $user->id);
    ...
    public function activateUser($code)
    {
        ...
        /** @var User $UserInfo */
        $UserInfo = User::find()->select('id,is_active,validate_email')->where([
            'email' => $data[2],
            'id' => $data[0],
        ])->one();
        if (empty($UserInfo)) {
    ...
    {
        if ($this->validate()) {
            $user = User::find()->select('id,email,created_at,validate_email')
            ...
            $actionService = Wc::$service->getAction();
            if ($actionService->checkLimit('send_active_email', User::tableName(), $user['id'])) {
                Wc::$service->getLog()->create('send_active_email', User::tableName(), $user['id'], $user['id']);
    ...
    {
        /** @var User $user */
        $user = $this->loadModel(User::className(), $uid, false);
        if ($user == null) {
        ...
        if ($user->save(false)) {
            Wc::$service->getLog()->delete('error_password', User::tableName(), $uid, $uid);
