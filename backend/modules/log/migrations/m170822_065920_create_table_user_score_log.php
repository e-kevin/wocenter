...
class m170822_065920_create_table_user_score_log extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $this->createIndex('idx-user_score_log-created_at', '{{%user_score_log}}', 'created_at');
        $this->createIndex('idx-user_score_log-model', '{{%user_score_log}}', 'model');

        $this->addForeignKey('fk-user_score_log-uid', '{{%user_score_log}}', 'uid', '{{%user}}', 'id', 'CASCADE');
        $this->addForeignKey('fk-user_score_log-type', '{{%user_score_log}}', 'type', '{{%user_score_type}}', 'id', 'CASCADE');
