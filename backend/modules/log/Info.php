<?php
namespace wocenter\backend\modules\log;

use wocenter\core\ModularityInfo;

class Info extends ModularityInfo
{

    public function init()
    {
        parent::init();

        $this->id = 'log';
        $this->name = '日志管理';
        $this->description = '管理系统所有日志，如：行为日志、积分日志';
        $this->version = '1.0';
        $this->developer = 'WoCenter';
        $this->email = 'e-kevin@qq.com';
        $this->isSystem = true;
        $this->defaultRoute = 'action';
    }

    /**
     * @inheritdoc
     */
    public function getMenus()
    {
        return [
            // 安全管理
            [
                'name' => '安全管理',
                'items' => [
                    [
                        'name' => '日志管理',
                        'icon_html' => 'list',
                        'modularity' => $this->id,
                        'show_on_menu' => true,
                        'items' => [
                            // 行为日志
                            [
                                'name' => '行为日志',
                                'url' => "/{$this->id}",
                                'full_url' => "/{$this->id}/{$this->defaultRoute}/index",
                                'show_on_menu' => true,
                                'items' => [
                                    ['name' => '列表', 'url' => "/{$this->id}/{$this->defaultRoute}/index", 'description' => '行为日志列表'],
                                    ['name' => '删除', 'url' => "/{$this->id}/{$this->defaultRoute}/delete"],
                                    ['name' => '批量删除', 'url' => "/{$this->id}/{$this->defaultRoute}/batch-delete"],
                                    ['name' => '搜索', 'url' => "/{$this->id}/{$this->defaultRoute}/search", 'description' => '搜索行为日志'],
                                ],
                            ],
                            // 奖罚日志
                            [
                                'name' => '奖罚日志',
                                'url' => "/{$this->id}/score",
                                'full_url' => "/{$this->id}/score/index",
                                'show_on_menu' => true,
                                'items' => [
                                    ['name' => '列表', 'url' => "/{$this->id}/score/index", 'description' => '奖罚日志列表'],
                                    ['name' => '搜索', 'url' => "/{$this->id}/score/search", 'description' => '搜索奖罚日志'],
                                ],
                            ],
                        ],
                    ],
                ],
            ],
        ];
    }

}
