...
class Config extends ActiveRecord
{
     ...
     */
    public static function getAllConfig($fromCache = true)
    {
        ...
        if ($arr === false) {
            $arr = Config::find()->select(['name', 'value', 'extra'])->where(['status' => 1])->indexBy('name')->asArray()->all();
            if ($arr !== null) {
                Yii::$app->getCache()->set($cachekey, $arr);
            }
        }
     ...
     */
    public static function getValue($key, $defaultValue = null, $fromCache = true)
    {
        $arr = self::getAllConfig($fromCache);
        if ($arr[$key] === null) {
     ...
     * @return array
     */
    public static function getExtra($key, $fromCache = true)
    {
        $arr = self::getAllConfig($fromCache);
        if ($arr[$key] === null) {
            return '不存在配置项：' . $key;
    ...
    public function clearCache()
    {
        ...
        Yii::$app->getCache()->delete(self::CACHE_PREFIX . 'all');
    }
