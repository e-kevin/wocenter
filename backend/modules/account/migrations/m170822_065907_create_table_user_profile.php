...
class m170822_065907_create_table_user_profile extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $this->createTable('{{%user_profile}}', [
            ...
            'last_location' => $this->char(20)->notNull()->comment('最后登录位置'),
        ...
        $this->batchInsert('{{%user_profile}}', [
            ...
            [1, '系统', '系统', 0, '0000-00-00', '', '', '', 4, '127.0.0.1', 1492590233, '127.0.0.1', 1503283442, '本机地址', NULL, 1, 1, 0, 0, 0, 0],
        ]);
        
        $this->createIndex('idx-user_profile-default_identity', '{{%user_profile}}', 'default_identity');
        
        $this->addForeignKey('fk-user_profile-uid', '{{%user_profile}}', 'uid', '{{%user}}', 'id', 'CASCADE');
        $this->addForeignKey('fk-user_profile-last_login_identity', '{{%user_profile}}', 'last_login_identity', '{{%identity}}', 'id', 'SET NULL');
